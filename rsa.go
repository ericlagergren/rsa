...
// Includes reports whether the accumulator probably includes v.
func (z *Accumulator) Includes(v []byte) bool {
	// Claim: u^x = w
	u := new(big.Int).Mod(z.u, z.n)
	//u := new(big.Int).Set(z.u)
	q, r := new(big.Int).DivMod(u, hashToPrime(v), new(big.Int))
	fmt.Printf("q=%d\n", q)
	fmt.Printf("r=%d\n", r)
	//r.Mod(r, z.n)
	return r.Sign() == 0
...
func (z *Accumulator) Update(c Change, w Witness) Witness {
	...
	var lhs, rhs big.Int
	rhs.Exp(o.c, &a, z.n) // rhs = A^a mod N
